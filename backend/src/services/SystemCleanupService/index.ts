...
const cleanMessagesAndTickets = async (
...
export const cleanupSystem = async (
  options: CleanupOptions
  ...
  try {
    await Promise.all([
      Setting.findOrCreate({
        ...
        defaults: {
          key: "cleanup.autoCleanup",
          value: String(settings.autoCleanup)
        }
      }).then(([setting]) => {
      ...
      Setting.findOrCreate({
        ...
        defaults: {
          key: "cleanup.olderThan",
          value: String(settings.olderThan)
        }
      }).then(([setting]) => {
      ...
      Setting.findOrCreate({
        ...
        defaults: {
          key: "cleanup.cleanTemp",
          value: String(settings.cleanTemp)
        }
      }).then(([setting]) => {
      ...
      Setting.findOrCreate({
        ...
        defaults: {
          key: "cleanup.cleanLogs",
          value: String(settings.cleanLogs)
        }
      }).then(([setting]) => {
      ...
      Setting.findOrCreate({
        ...
        defaults: {
          key: "cleanup.cleanMedia",
          value: String(settings.cleanMedia)
        }
      }).then(([setting]) => {
      ...
      Setting.findOrCreate({
        ...
        defaults: {
          key: "cleanup.cleanClosedTickets",
          value: String(settings.cleanClosedTickets)
        }
      }).then(([setting]) => {
