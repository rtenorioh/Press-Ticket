...
const ListTicketsService = async ({
  ...
  try {
      ...
      if (!isAdmin) {
        if (allTicketsEnabled) {
          if (queueIds && queueIds.length > 0) {
            ...
            whereCondition = {
              ...whereCondition,
              [Op.or]: [{ userId: userId }, { queueId: null }]
            };
          }
        } else {
          if (queueIds && queueIds.length > 0) {
        ...
        if (queueIds && queueIds.length > 0) {
          whereCondition = {
            ...
            [Op.or]: [
              { userId: userId },
              {
                [Op.and]: [{ queueId: { [Op.in]: queueIds } }, { userId: null }]
              }
            ]
          };
        } else {
    ...
    const offset =
      all === "true" ? 0 : defaultLimit * (+(pageNumber || "1") - 1);

    const listSettingsService = await ListSettingsServiceOne({ key: "ASC" });
    const settingASC =
      listSettingsService?.value === "enabled" ? "ASC" : "DESC";

    const listSettingsService2 = await ListSettingsServiceOne({
      key: "created"
    });
