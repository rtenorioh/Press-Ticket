...
const EmitTicketCounterService = {
  async execute(counters: TicketCounters): Promise<void> {
    try {
      ...
      logger.info(
        `[BACK_COUNTERS_EMIT][${timestamp}] Emitindo evento de atualização de contadores`,
        {
          counters
        }
      );
      ...
      logger.info(
        `[BACK_COUNTERS_SUCCESS][${timestamp}] Evento de atualização de contadores emitido com sucesso`
      );
    } catch (err) {
