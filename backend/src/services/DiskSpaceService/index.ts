...
export const getFolderContents = async (
  ...
  return new Promise<FolderSizeInfo[]>((resolve, _reject) => {
    exec(`ls -la "${fullPath}" 2>/dev/null`, (error, stdout, _stderr) => {
      if (error) {
        resolve([]);
        return;
      }
      ...
      for (const line of lines) {
        ...
        const duCommand = isDirectory
          ? `du -sb "${itemFullPath}" 2>/dev/null`
          : `stat -c%s "${itemFullPath}" 2>/dev/null`;

        exec(duCommand, (error, stdout, _stderr) => {
          if (error) {
            processedCount++;
            if (processedCount === lines.length) {
              resolve(items.sort((a, b) => b.sizeBytes - a.sizeBytes));
...
export const getDiskSpaceInfo = async (): Promise<DiskSpaceInfo> => {
    ...
    return new Promise<FolderSizeInfo[]>((resolve, reject) => {
      const buildTree = async (
        ...
        return new Promise((resolve, _reject) => {
          exec(`ls -la "${dirPath}" 2>/dev/null`, (error, stdout, _stderr) => {
            if (error) {
              resolve([]);
              return;
            }
            ...
            for (const line of lines) {
              ...
              const duCommand = isDirectory
                ? `du -sb "${fullPath}" 2>/dev/null`
                : `stat -c%s "${fullPath}" 2>/dev/null`;

              exec(duCommand, async (error, stdout, _stderr) => {
                if (error) {
                  processedCount++;
                  if (processedCount === lines.length) {
                    resolve(items.sort((a, b) => b.sizeBytes - a.sizeBytes));
  ...
  const getFolderSize = () => {
    return new Promise<{ size: string; bytes: number }>((resolve, reject) => {
      exec(`du -sh ${folderPath}`, (error, stdout, _stderr) => {
        if (error) {
        ...
        exec(`du -sb ${folderPath}`, (error, stdout, _stderr) => {
          if (error) {
            logger.error(
              `Erro ao obter tamanho da pasta em bytes: ${error.message}`
            );
            reject(error);
            return;
          }
  ...
  const getDiskFreeSpace = () => {
    return new Promise<{
      ...
      exec("df -h /", (error, stdout, _stderr) => {
        ...
        exec("df --output=size,avail / | tail -1", (error, stdout, _stderr) => {
          if (error) {
