...
export const updateWhatsappLibFromGit =
  async (): Promise<WhatsappLibGitUpdateResult> => {
    try {
      ...
      packageJson.dependencies["whatsapp-web.js"] =
      ...
      try {
        ...
        throw new Error(
          `Falha ao instalar a biblioteca do GitHub: ${installError.message}`
        );
      }
    } catch (error) {
      logger.error(`Erro ao atualizar whatsapp-web.js do GitHub: ${error}`);
      return {
        success: false,
