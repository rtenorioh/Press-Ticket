...
rateLimitRoutes.get("/rate-limit-status", (req: Request, res: Response) => {
  ...
  const response = {
    ...
    currentConfig: {
      mode: isStrictMode
        ? "🔴 Modo Estrito (RATE_LIMIT_STRICT=true)"
        : isDevelopment
          ? "🟢 Desenvolvimento"
          : "🟡 Produção Normal",
    ...
    howToTest: {
      step1:
        "Faça múltiplas requisições para /auth/login com credenciais inválidas",
    ...
    tips: {
      blocked:
        "Se bloqueado, aguarde 15 minutos ou reinicie o servidor (pm2 restart backend)",
      strictMode: "Para ativar modo estrito: RATE_LIMIT_STRICT=true no .env",
      development:
        "Em desenvolvimento, os limites são mais permissivos para facilitar testes"
    }
  };
  ...
  (req: Request, res: Response) => {
    ...
    const response = {
      ...
      rateLimit: {
        ...
        resetDate: reset
          ? new Date(parseInt(reset.toString()) * 1000).toISOString()
          : null,
      ...
      tips: {
        ...
        blocked:
          remaining && parseInt(remaining.toString()) === 0
...
rateLimitRoutes.post("/rate-limit-reset", (req: Request, res: Response) => {
  ...
  return res.json({
    message:
      "⚠️ Para resetar o rate limit em desenvolvimento, reinicie o servidor:",
    commands: ["pm2 restart backend", "ou", "npm run dev"],
